...
export function WalletModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const { isAuthenticated, address, switchWallet } = useAuth()
  if (!open) return null
  return (
        ...
        <div className="w-full max-w-3xl rounded-2xl border border-white/15 bg-white/60 dark:bg-black/30 backdrop-blur-xl shadow-2xl neon-border glass-card overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
            <div className="flex items-center gap-2">
              ...
              {isAuthenticated && (
                <span className="ml-2 text-xs opacity-80 font-mono hidden sm:inline">{address?.slice(0,6)}…{address?.slice(-4)}</span>
              )}
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="p-5">
            {tab === 'overview' ? (
              <div className="glass-card neon-border glass-hover shimmer anim-slide-in p-0">
                <div
                  className="rounded-xl border border-white/10 p-4"
                  style={{
                    background: 'linear-gradient(135deg, rgba(0, 0, 0, 0.55), rgba(24,24,28,0.55))'
                  }}
                >
                  <WalletOverview />
                </div>
              ...
              <div className="glass-card neon-border glass-hover shimmer anim-slide-in p-0">
                <div
                  className="rounded-xl border border-white/10 p-4"
                  style={{
                    background: 'linear-gradient(135deg, rgba(60,60,66,0.55), rgba(24,24,28,0.55))'
                  }}
                >
                  <AccountHealth />
                </div>
